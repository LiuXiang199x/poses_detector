...
class TutorialBodySkill(Skill):
	...
	def __init__(self):
		...
		self._body_gestures = [ "relajando", "cansado", "no", "si", "hola", "hola_asia","animar", 
							"sentarse", "levanta_brazo", "esperar", "continuar", "esperar", "de_pie"]
		...
		self._gesture2user = random.sample(self._body_gestures, 5)
		#self.create_msg_srv()
	...
	def create_msg_srv(self):
		...
		## ROS Subscribers: receive the number of person
		...
		rospy.loginfo("[START TUTORIAL_BODY SKILL]")
	...
	def state_transition(self):
		...
		elif self.__app_state == "waiting_detect_user":
			self.__app_state = "no_detect_user"
	...
	def detect_user(self):
		if self._num_person == 0 and not self.__skill_finished:
			self._tmp_state.append(self.__app_state )
			self.__app_state = "nobody_detected"
			if len(self._tmp_state) == 4:
				self.__app_state ="tutorial_finish"
		print(self._tmp_state)
		if not len(self._tmp_state) == 0 and self._num_person > 0:
			self.__app_state = self._tmp_state[0]
			del self._tmp_state[:]
	...
	def execute_cb(self, goal):
		...
		if self._status == self.RUNNING:
			...
			self._tmp_pose = self.__app_state
			self.percentage_completed = 0
			self.engagement = True
			self.__build_feedback()
			...
			try:
				...
				while not self.__skill_finished:
					...
					self._target_body_gesture = self._gesture2user[self._gesture_index]
	...
	def app_transitions_state(self):
		...
		if self.__app_state == "end_app":
			...
			t = "Bienvenido al tutorial de reconocimiento de pose corporal. Colocate a una distancia correcta"
			#menu_value = "Sí|No"
			#menu_type = "text"
			...
			t = "Se acabó tutorial del reconocimiento del cuerpo, muchas gracias por su participación."
			...
			if len(self._tmp_state):
				...
				t = "Muy bien ubicado. Vamos a empezar los ejercicios y ompleta la pose que se muestra en la imagen."
				...
				t = "Vamos empezar a hacer ejecicio " + self._target_body_gesture
				...
				# self._image_path = "/video/haobing/Levantamiento lateral de los brazos.mp4"
				self._image_path = "/image/xiang/body/"+self._target_body_gesture+".png"
				...
				menu_value = "Sí|No"
				menu_type = "text"
				continue_exercise = "¿Desea repetir el ejercicio"+self._target_body_gesture
				...
				self._target_body_gesture = self._body_gestures[self._gesture_index]
